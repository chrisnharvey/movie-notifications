...
class Dvd extends Controller {
	...
	public function index()
	{
		...
		if(isset($all_good))
		{
			...
			$prefs = array("show_next_prev" => TRUE, 'next_prev_url'   => site_url("dvd/"));
			...
			else
			{
				...
				$days = $this->db->get_where("releases", array("type" => "DVD"), 1);
				...
				if($days->num_rows() > 0)
				{
					...
					{
						$prefs['template'] .= '<th>'.anchor(site_url("dvd/".$days->row()->year."/".$days->row()->month), '&lt;&lt;').'</th>';
					}
				}
				else
				{
				...
				$days = $this->db->get_where("releases", array("YEAR(date)" => $year, "MONTH(date)" => $month, "type" => "DVD"));
	
				$cal_data = array();

				foreach($days->result() as $cal_day)
				{
					$cal_data[$cal_day->day] = site_url("dvd/".$year."/".$month."/".$cal_day->day);
				}
				...
				if(isset($day))
				{
					...
					else
					{
						...
						$query = $this->db->get_where("releases", array("YEAR(date)" => $year, "MONTH(date)" => $month, "DAY(date)" => $day, "type" => "DVD"));
						...
						$data['title'] = "DVD's released on ".date("jS F Y", strtotime($query->row()->date));
						$data['movies'] = $movies;
					}
				}
				else
				{
						...
						$query = $this->db->get_where("releases", array("YEAR(date)" => $year, "MONTH(date)" => $month, "type" => "DVD"), 10);
						...
						$data['title'] = "DVD's released in ".date("F Y", strtotime($query->row()->date));
						$data['movies'] = $movies;
				}
			}
		}

		$this->page->title = "On DVD";
		$this->page->show('dvd', $data);
	}
